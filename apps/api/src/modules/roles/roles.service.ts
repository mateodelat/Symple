...
export class RolesService {
  ...
  async checkUserHasAccessToDepartment(department: string): Promise<void> {
    ...
    const hasAccess =
      await this.departmentsService.checkUserHasAccessToDepartment({
        user,
        department,
      });
    if (!hasAccess)
  ...
  async create(payload: CreateRoleDTO): Promise<Role> {
    ...
    const department = await this.departmentsService.checkDepartmentExistsById(
      departmentId,
    );

    const roles =
      department?.roles instanceof Array
        ? [...department.roles, newRole.id]
        : [newRole.id];
